...
**F9 PCO 商品目录导入+补货监控计划 v2 已产出（2026-09-10，v3.7.0 提案，待审核）：见 [docs/PLAN-F9-CATALOG-IMPORT.md](docs/PLAN-F9-CATALOG-IMPORT.md)**（数据源改为 PCO 本体：图/链接/名称/价格/限购五要素；路线=真实浏览器执行——书签脚本手动采集 + aoi-pco-monitor 独立项目 Playwright 定时监控（新活动/补货 diff + QQ 通知，已批准立项）；LLM 完全舍弃；小程序导入模板已解析；回流方向不开发）；
...
| `docs/PLAN-F9-CATALOG-IMPORT.md` | F9 PCO 商品目录导入 + 补货监控：**计划 v2（2026-09-10，v3.7.0 提案，待审核）**——书签脚本手动导入 + aoi-pco-monitor（Playwright）定时监控 |
