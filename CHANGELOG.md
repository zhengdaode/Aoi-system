...
> 从链接导入订单：排谷表/汇总表分享直链一键导入（独立小版本，与 F6 统计 demo 无关；F6 仍待管理层审核）。
...
### Tests
- import.test.js 6 → 13 用例（全套 179 全绿）：zwlhome 排谷表/汇总表真实结构解析回归 ×2；链接拉取层 `mapProxyUrl`（白名单外返回 null）/`fileNameFromUrl`/`fetchFromUrl` 通道优先、HTML 回退页跳过、全失败引导 ×5；另以 Node 直连两份线上真实链接端到端验证 30×2 条订单解析一致
