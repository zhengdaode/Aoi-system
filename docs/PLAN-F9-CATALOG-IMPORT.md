# F9 · PCO 商品目录导入 + 补货监控 —— 实施计划 v3.1（v3.7.0 提案，待审核通过后实施）
...
**同步快速形态（可选升级，默认不做）**：monitor 若改为 NapCat 机常驻（SSH 可登录），可暴露同步 HTTP 抓取端点，「输入链接」15–30 秒出结果（Actions 异步形态约 1–2 分钟）；待负责人对速度有实际需求再立项。
