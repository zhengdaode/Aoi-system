...
> ③F6 改为「独立页面本地化 demo」形态（见 F6 节）；④F7 / F8 取消不再进行。
...
### F6（形态修订 2026-09-08）团期复盘统计 · 独立本地 demo

- **新形态**：不接入主系统、不动现有代码——单独一个**本地 HTML demo**（浏览器直接打开，内置合成数据，
  不连 Supabase），只实现基本前端 UI，**供管理层审核功能形态**；审核通过后再另行决定并入主系统的方式
  与版本。
- demo 内容（静态展示）：按活动/团期聚合订单数、金额（人民币口径）、币种分布、交费回收率、发货时效，
  按 IP 排行；布局复用 editorial 设计规范（docs/design/DESIGN.md）。
...
| v3.6.0 | 治理与复盘 | B5（迁移版本化 + 自检）+ B6（blob 治理）+ F6 demo（按管理层审核反馈决定并入方式） | ~2 天 |
