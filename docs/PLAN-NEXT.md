...
>
> **执行纪律（2026-09-08 用户补充）**：①每一步改动独立成 commit，保证可随时单独回退；
> ②F5 另立独立项目，实施前先产出详细功能设计供用户审核，审核通过后分步实现，**本轮迭代不实施**；
> ③F6 形态历经两次修订（demo → 按用户决策取消 demo 直接并入主系统，2026-09-08 已实装 v3.5.0，见 F6 节）；④F7 / F8 取消不再进行。
...
- **Phase 1（低成本，v3.4.0 建议）**：
  ...
  3. 密钥强度升级到 `encode(gen_random_bytes(16),'hex')`（32 字符），旧密钥保留兼容、下次重新生成时生效；
     UI 提示团长「重新生成后将旧密钥作废」。
- **Phase 2（不排期）**：商用化路线已暂缓（见 F8 节的 2026-09-08 决策）；「地址/QQ/凭证拆 `member_pii`
  独立表、行级隔离 (team_id, cn)」保留为设计点，未来若启动商用再排期。
...
### F5（✅ 2026-09-09 已实施）QQ 机器人双向：独立仓库 aoi-qqbot M1–M7

- **2026-09-08 用户决策**：从本迭代路线中摘出，**单独开设新项目**；实施前先产出详细功能设计文档
  （交互流程 / 指令集 / relay 双向改造 / 鉴权与防滥用 / 隐私边界），**经用户审核确认要实现的功能后**
  再分步实现。本轮迭代（v3.4.0–v3.6.0）不做任何 F5 相关改动，原「低配版（通知带链接）」一并移出。
- **✅ 已实施（2026-09-09）**：详细设计（[docs/PLAN-F5-QQBOT-BIDIRECTIONAL.md](PLAN-F5-QQBOT-BIDIRECTIONAL.md)）经三轮审核 v3 定稿后，在独立仓库 [aoi-qqbot](https://github.com/zhengdaode/aoi-qqbot) 按 M1–M7 分步实施（每里程碑独立 commit + 测试）；主仓库接入改动见 CHANGELOG v3.5.3
  —— 功能清单 / 架构增量 / 安全防滥用 / M1–M4 分步计划 / 六项待拍板问题。

### F6（✅ 2026-09-08 已实装 v3.5.0）团期复盘统计 · 并入主系统

- **2026-09-08 用户决策**：demo 路线取消（不再迭代 demo、不要求管理层审核），**直接并入主系统**；
  实施前先以审核者视角自行迭代功能口径（回收率改金额口径、KPI 扩 9 项、新增买家排行/币种分布/发货时效/口径切换等 8 项结论）。
- **迭代结论与实现方案**：见 [docs/PLAN-F6-STATS.md](PLAN-F6-STATS.md)。
- **落地内容**：新模块 `js/stats.js`（第 17 个 js 模块，只读统计）+ sidebar「工具 → 复盘统计」页
  （团期/IP 筛选、9 项 KPI、按活动聚合（可导出）、IP/买家排行（口径切换）、币种分布、金额口径交费回收、按批次发货时效）；
  数据埋点 `orders.shippedAt`（首标已发时写入，旧数据优雅排除，无需迁移）；交费口径与审批页 `buyerSummary` 同源。
- `demo/stats-demo/` 与 `demo/F6-团期复盘统计-demo.zip` 保留为历史产物，可另行删除。
...
| 版本 | 主题 | 内容 | 量级 |
|------|------|------|------|
| **v3.4.0（✅ 2026-09-08 已实施完成）** | 数据安全兜底 | B1（历史表+备份 RPC）+ F4（备份 UI）+ B2 Phase 1（PII 剥离/写白名单/密钥升强）+ F1（审批/到货通知）+ F2（团员时间线）——共 11 个 commit，测试 170/170，线上库已应用并探针验证 | 完成 |
| **v3.5.0（✅ 2026-09-08 已实施完成）** | F6 复盘统计 + 从链接导入 | F6 团期复盘统计并入主系统（js/stats.js + 复盘统计页 + orders.shippedAt 埋点，见 docs/PLAN-F6-STATS.md）+ 从链接导入（计划外新增：直链代理拉取排谷表/汇总表）；demo 路线取消 | 完成 |
| v3.6.0 | 链路加固与追责 | B4（推送审计 + UPSTREAM 移 env + CSP/6099 收尾）+ B3（登录防爆破 + 审计日志）+ F3（汇率同步）（原 v3.5.0 内容顺延） | ~2 天 |
| v3.7.0 | 治理 | B5（迁移版本化 + 自检）+ B6（blob 治理）（原 v3.6.0 内容顺延；F6 已提前随 v3.5.0 落地） | ~1.5 天 |
| ~~v4.0.0~~ | 已取消 | 2026-09-08 用户决策：商用化路线（B2 Phase 2 / B7 / 多团）不再排期；F7 / F8 一并取消；设计点保留不删除 | — |
...
- **F5 QQ 机器人双向**：另立独立项目，详细功能设计先经用户审核再分步实现（见 F5 节）。
- **F7 黑夜模式移植 / F8 多团商用化**：2026-09-08 用户决策取消。F8 取消后，B2 Phase 2（PII 拆表）与
  B6 单团显式化仍保留——前者是设计储备，后者本就是防御性修正，与商用决策无关。
