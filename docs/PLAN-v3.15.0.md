# PLAN v3.15.0（已实施完成，2026-09-13）— 商品联动闭环 + 活动级价格生成与导出 + PCO 目录瘦身 + 复盘性能治理
...
> ⑨AI 提示词优化争取一次性导入中文版。**2026-09-13 负责人审阅批准：D1=删数据+代码归档，但保留后续重新访问路径（删前快照 + `--restore` 恢复入口）；D2=通过（订单管理原入口移除）；D3=追问「为何不能通过提示词限制」→ 结论：粘贴时 `<img>` 属性在进入模型前已被 ChatGPT 客户端剥离，提示词无法约束模型收不到的信息；S9 调整为提示词 8 列 + 联网浏览取图分支 + 我端 URL 校验防幻觉。按切片实施。**
