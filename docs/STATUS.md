...
> 更新日期：2026-09-19 · 版本 **v3.17.2**（v3.17.2 每人应付国际费「购买内容」同商品合并为一条累计件数；
> v3.17.1 每人应付国际费「购买内容」列宽随视口自适应换行；
> v3.17.0 国际计算批次化四项体验：导出文件名批次前缀 / 恢复订单
> 管理勾选版批量生成人民币价 / 每人应付国际费·货物件数（管理端+团员端）/ 订单管理勾选统计；
> v3.16.0 订单商品详情弹窗 + 全站按钮配色收敛（仅按钮：
> 珊瑚主操作/中性灰次要/红危险三款，链接与状态徽章不动）；v3.15.2 手动录入订单商品同步登记主档；v3.15.1 部署收尾：relay v4.1 +
...
## 当前状态速览（2026-09-19 v3.17.2 后）

- **代码层**：v3.17.2 修复轮；vitest 全套 **467 例全绿**，aoi-qqbot node:test **68 例全绿**。前端两通道
...
- **v3.17.2 要点**：每人应付国际费「购买内容」同商品合并——`Aoi.intl.buyerRows` 按 `type|model` 键聚合
  （件数累加、保持首现顺序），同一买家同商品多笔订单显示为一条「类型-型号 ×合计数」，与分摊明细口径对齐；
  金额/件数计算不受影响，导出自动跟随。
- **v3.17.0 要点**：国际计算页「每人应付国际费」「分摊明细」导出文件名加批次前缀（`data-batch-from`，
  如 `8.23涩谷-每人应付国际费.png`）；每人应付国际费增列「货物件数」（每人批次件数，紧邻国际金额）+
  分摊明细统计行「合计 N 件」；团员端「我的国际费」增列「我的件数」（收货核对）；订单管理恢复勾选版
  「批量生成人民币价」（与活动管理级生成并存）并新增「统计勾选」（物件总数量 / 本体总金额·原货币+人民币 /
  国际邮费 / 整体总金额）。
- **v3.16.0 要点**：订单管理「型号」列可点击 → 商品详情弹窗（主档 `activityProduct`→`productFillFor` 逐级回退
  订单字段：原名/参考价/外币原价/参考图/限购 + 当前订单行；展开区 图片链接/购买地址/平台链接超链接直跳；
  ×/遮罩/Esc 关闭；≤640px 贴底 bottom sheet）；全站按钮配色收敛——`.btn-primary`/`.btn-outline` 珊瑚两款
  接替 47 处蓝绿靛紫实底与蓝描边，导航激活 `.nav-active`，危险红统一 red-600，链接/徽章/装饰按约不动。
